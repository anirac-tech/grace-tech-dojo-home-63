...
const Hero = () => {
  ...
  return (
    <section className="flex items-center pt-20 pb-28 overflow-hidden" aria-labelledby="hero-heading">
      {/* Background decoration */}
      <div 
        className="absolute inset-0 bg-gradient-to-tr from-dojo-950 via-dojo-900 to-dojo-800 z-0"
        aria-hidden="true"
      />
      <div 
        className="absolute inset-0 opacity-30 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.1)_0,rgba(255,255,255,0)_70%)]"
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 w-[calc(100%-30px)]">
        <div 
          ...
          className="transition-transform duration-200 ease-out mx-auto max-w-3xl"
        >
          <h1 
            id="hero-heading"
          ...
          <p className={`text-lg md:text-xl text-gray-300 mb-6 leading-relaxed transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className={`transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="mt-8 inline-block relative overflow-hidden group">
              <span 
                className="absolute inset-0 w-full h-full bg-gradient-to-r from-dojo-300/40 to-dojo-500/40 -translate-x-full group-hover:translate-x-0 transition-transform duration-300"
                aria-hidden="true"
              ></span>
              <button 
                className="relative z-10 bg-transparent border border-dojo-300 text-dojo-300 hover:text-white px-8 py-3 rounded-md overflow-hidden transition-colors duration-300 focus-outline"
      ...
      <div 
        className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-gray-900 to-transparent z-0"
        aria-hidden="true"
      />
    </section>
  );
